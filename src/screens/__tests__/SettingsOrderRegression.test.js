...
describe('Settings section order — Gradle preflight integration', () => {
  test('build-production.mjs patches build.gradle with preflightProduction task', () => {
    // android/app/build.gradle is gitignored and regenerated by
    // expo prebuild. The build-production.mjs wrapper patches it
    // with the preflight task after prebuild. Verify the wrapper
    // contains the patching logic.
    const wrapperPath = path.join(__dirname, '..', '..', '..', 'scripts', 'build-production.mjs')
    const wrapper = fs.readFileSync(wrapperPath, 'utf8')

    // The wrapper must define the preflight task
    expect(wrapper).toContain('preflightProduction')

    // The wrapper must wire it to bundleRelease
    expect(wrapper).toContain('bundleRelease')

    // The wrapper must wire it to assembleRelease
    expect(wrapper).toContain('assembleRelease')

    // The wrapper must configure the dependency
    expect(wrapper).toContain("dependsOn 'preflightProduction'")
  })
