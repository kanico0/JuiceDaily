...
describe('Post-login camera initialization', () => {
  test('10. onAuthenticated does not call attemptCameraOpen', () => {
    const source = fs.readFileSync(
      path.resolve(ROOT, 'src/screens/HomeScreen.js'),
      'utf-8'
    )
  ...
  test('11. useCamera hook clears timers on unmount', () => {
    const source = fs.readFileSync(
      path.resolve(ROOT, 'src/hooks/useCamera.ts'),
      'utf-8'
    )
    // Cleanup on unmount
  ...
  test('13. onCameraReady cancels the timeout', () => {
    ...
    const match = source.match(/const onCameraReady = useCallback\(\(\) => \{([\s\S]*?)\}, \[\]\)/)
    expect(match).toBeTruthy()
    expect(match[1]).toContain('clearTimeout(readyTimerRef.current)')
    expect(match[1]).toContain('readyTimerRef.current = null')
  })
  ...
  test('14. resetCamera clears timer and resets state', () => {
    ...
    const match = source.match(/const resetCamera = useCallback\(\(\) => \{([\s\S]*?)\}, \[\]\)/)
    expect(match).toBeTruthy()
    expect(match[1]).toContain('clearTimeout(readyTimerRef.current)')
    expect(match[1]).toContain('readyTimerRef.current = null')
